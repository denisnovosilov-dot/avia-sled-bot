...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "add":
        ...
        await add_subscription_handler(update, context)
    elif data == "list":
        await list_subscriptions(update, context)
    elif data == "help":
        await help_command(update, context)
    elif data == "pause_all":
        # Простая заглушка: приостановить все подписки пользователя
        subs = get_subscriptions(active_only=False)
        user_subs = [s for s in subs if s[1] == update.effective_user.id]
        for s in user_subs:
            toggle_subscription(s[0], update.effective_user.id, 0)
        await query.edit_message_text("Все ваши подписки приостановлены.")
    else:
        await query.edit_message_text("Неизвестная команда.")

async def add_subscription_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Если вызов из callback, то update - это CallbackQuery, нужно получить сообщение
...
async def list_subscriptions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_subs = [s for s in subs if s[1] == update.effective_user.id]
    if not user_subs:
        # Если вызвано из callback, используем query.edit_message_text, иначе reply_text
        if update.callback_query:
            await update.callback_query.edit_message_text("У вас нет активных подписок.")
        else:
